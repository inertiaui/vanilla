...
describe('helpers', () => {
    ...
    describe('except', () => {
        ...
        it('should support case insensitive matching for objects', () => {
            const obj = { Name: 1, AGE: 2, city: 3 }
            expect(except(obj, ['name', 'age'], true)).toEqual({ city: 3 })
        })

        it('should support case insensitive matching for arrays', () => {
            const arr = ['Name', 'AGE', 'city']
            expect(except(arr, ['name', 'age'], true)).toEqual(['city'])
        })
    })
    ...
    describe('only', () => {
        ...
        it('should support case insensitive matching for objects', () => {
            const obj = { Name: 1, AGE: 2, city: 3 }
            expect(only(obj, ['name', 'city'], true)).toEqual({ Name: 1, city: 3 })
        })

        it('should support case insensitive matching for arrays', () => {
            const arr = ['Name', 'AGE', 'city']
            expect(only(arr, ['name', 'city'], true)).toEqual(['Name', 'city'])
        })
    })
    ...
    describe('isStandardDomEvent', () => {
        ...
        it('should handle pointer events', () => {
            expect(isStandardDomEvent('onPointerDown')).toBe(true)
            expect(isStandardDomEvent('onPointerUp')).toBe(true)
            expect(isStandardDomEvent('onPointerMove')).toBe(true)
        })
